...
def _aff_bwd_kernel(
    ...
    for j_blocks in range(tl.cdiv(L, BLOCK_J) - 1, -1, -1):
        ...
        for d_offset in range(0, D, BLOCK_D):
            ...
            affinity_1 += tl.dot(k_mat*src_mat, tl.trans(kt_mat*dest_mat), input_precision="ieee")

        affinity_2 = tl.maximum(affinity_1, 0.0)
        affinity_3 = tl.exp2(tl.log2(affinity_2 + err) * 2.0 / 3.0)

        # daffinity = tl.load(
        #     daff_ptr + offs_i[:, None] * str_daff_li + offs_j[None, :] * str_daff_lj, 
        #     mask=(offs_i[:, None] < L) & (offs_j[None, :] < L), 
        #     other=0.0
        # ).cast(tl.float32)
        daffinity = tl.load(
            daff_ptr + offs_j[:, None] * str_daff_li + offs_i[None, :] * str_daff_lj, 
...
def _aff_bwd_row_kernel(
    ...
    for j_offset in range(0, L, BLOCK_J):
        ...
        dsrc_acc += tl.sum(dac_mat * affinity_1, axis=1) 
        ...
        dk_i_acc += tl.dot(dac_mat, kt_mat*dest_mat, input_precision="ieee")

    dsrc_vec = tl.div_rn(dsrc_acc, 2.0 * (src_vec + err))
    tl.store(
...
def _aff_bwd_col_kernel(
    ...
    for i_offset in range(0, L, BLOCK_I):
        ...
        ddest_acc += tl.sum(dac_mat * affinity_1, axis=0) 
        ...
        dk_j_acc += tl.dot(tl.trans(dac_mat), k_mat*src_mat, input_precision="ieee")

    ddest_vec = tl.div_rn(ddest_acc, 2.0 * (dest_vec + err))
    tl.store(
...
def _affinity_bwd(k, src, dest, daff):
    ...
    #     daff_t = daff.transpose(-1, -2).contiguous()
    # daff_t.record_stream(s1)
    # daff_t.record_stream(s_i)
    # daff_t.record_stream(s_j)
    ...
    daff_cs.record_stream(s1)
    daff_cs.record_stream(s_i)
    daff_cs.record_stream(s_j)
    ...
    dk_i.record_stream(s_i)
    dsrc.record_stream(s_i)
    ...
    with torch.cuda.stream(s_j):
        ...
        dk_j.record_stream(s_j)
        ddest.record_stream(s_j)
...
def get_optimal_configs():
    ...
    print("_aff_bwd_row_kernel")
    print(_aff_bwd_row_kernel.best_config)
